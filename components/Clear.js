...
import { Container, Header, Content, List, ListItem, Thumbnail,Left, Body, Right, Button,CheckBox,Input,Root,Footer} from 'native-base';
    ...
    render(){
        ...
        if(data.length>0){
            data.forEach(element => {
                buymoney =buymoney+element.price*element.quantity;
             });
        }
      ...
      return <Container style={styles.box}>
            <ScrollView style={{ flex: 1 }}>
                ...
                <View style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center", backgroundColor: "white", padding: 10 }}>
                  ...
                  <View style={{ width: 280 }}>
                    <View style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", marginBottom: 10 }}>
                      <Text style={{ fontSize: 14 }}>收货人：daykalif</Text>
                      <Text style={{ fontSize: 14 }}>18645290046</Text>
                    </View>
                    <Text style={{ fontSize: 12 }}>
                      收货地址：杭州市江干区九堡镇九堡街道金雅苑小区27幢9单元501
                    </Text>
                    <Text
                      style={{
                        fontSize: 12,
              ...
              {data.map(element => (<View key={element._id+1}>
                  <View style={{ display: "flex", flexDirection: "row", alignItems: "center", backgroundColor: "white", marginTop: 4, paddingHorizontal: 10, marginTop: 5 }}>
                    ...
                    <Text
                      style={{ height: 36, lineHeight: 36, fontSize: 12 }}
                    >
                      天猫旗舰店
                    </Text>
                  </View>
                  ...
                  <View style={styles.everybaby}>
                    <Image source={{ uri: element.img }} style={{ width: 100, height: 100 }} />
                    <View style={{ marginLeft: 10, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                      <Text numberOfLines={2} style={{ width: 220, fontSize: 12 }}>
                        {element.content}
                      </Text>
                      <View>
                        <Text style={{ fontSize: 12 }}>
                  ...
                  <View>
                    <View>
                      ...
                      <View style={[styles.equity, styles.subtotalequity]}>
                        <Text style={{ marginRight: 10, fontSize: 12 }}>
                          共{element.quantity}件商品小计:
                        </Text>
                        <Text>￥ {element.price * element.quantity}</Text>
                      </View>
                    </View>
                  </View>
                </View>))}
            ...
            <View style={styles.bottom}>
              ...
              <Text style={{ height: 50, lineHeight: 50, backgroundColor: "orange", color: "white", textAlign: "center", width: 100, marginLeft: 10 }} onPress={() => {
